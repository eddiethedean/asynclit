# asynclet — package specification

## 1. Overview

**asynclet** is an async task layer aimed at [Streamlit](https://streamlit.io/) (and similar environments with a synchronous UI thread and rerun-driven updates). It provides:
...
- Optional hooks for **session-scoped** task registries and future **scheduler** integration  
...
- **`asynclet.run(..., manager=None)`** uses a process-wide **default manager** from **`get_default_manager()`** when `manager` is omitted.  
...
```python
task = asynclet.run(func, /, *args, manager=None, **kwargs)
```
...
```python
asynclet.session_tasks(session_state, key="asynclet_tasks")
asynclet.get_default_manager()
asynclet.TaskManager(max_completed=256)
```
...
- **Future note:** cooperative cancellation with **anyio** cancel scopes is a possible extension; the current implementation is **asyncio-centric**.  
...
### Delivered in 0.1.x (baseline)

- Basic task execution on a background loop  
- Sync (`asyncify`) and async callables  
- Polling API (`done`, `result`, `status`, `error`)  
- Progress streaming (Janus) and cancellation  
- Session helper and optional scheduler extra  

### Future (0.2+)

- First-class **APScheduler** integration helpers  
- **Retry** policies and backoff  

### Future (1.0+)

- Optional **distributed** backends (e.g. Celery / Ray)  
- **Plugin** hooks for transports (WebSocket, etc.)  
- Richer **reactive** UI bindings  
...
- Celery / Ray backends  
- WebSocket push instead of pure polling  
- Reactive bindings to widget state  

---

## 16. Conclusion

**asynclet** adds a practical async/sync task layer for Streamlit-style apps: one worker loop, poll-friendly **`Task`** handles, optional progress and cancellation, and room to grow toward schedulers and distributed workers without changing the core mental model.
